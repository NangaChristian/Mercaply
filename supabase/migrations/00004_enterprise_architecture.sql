...
-- ==============================================================================
-- 3.5. UPGRADE EXISTING TABLES
-- ==============================================================================

-- Upgrade PRODUCTS table
...
ALTER TABLE public.products ADD COLUMN IF NOT EXISTS currency_code CHAR(3) DEFAULT 'XAF' REFERENCES public.currencies(code);
...
ALTER TABLE public.products ADD COLUMN IF NOT EXISTS search_vector tsvector GENERATED ALWAYS AS (setweight(to_tsvector('french', coalesce(title, '')), 'A') || setweight(to_tsvector('french', coalesce(description, '')), 'B')) STORED;

-- Upgrade SERVICES table
ALTER TABLE public.services ADD COLUMN IF NOT EXISTS company_id UUID REFERENCES public.companies(id) ON DELETE CASCADE;
ALTER TABLE public.services ADD COLUMN IF NOT EXISTS subcategory_id UUID REFERENCES public.subcategories(id);
ALTER TABLE public.services ADD COLUMN IF NOT EXISTS slug TEXT;
ALTER TABLE public.services ADD COLUMN IF NOT EXISTS base_price NUMERIC(12,2);
ALTER TABLE public.services ADD COLUMN IF NOT EXISTS price_type TEXT;
ALTER TABLE public.services ADD COLUMN IF NOT EXISTS currency_code CHAR(3) DEFAULT 'XAF' REFERENCES public.currencies(code);
...
ALTER TABLE public.services ADD COLUMN IF NOT EXISTS rating NUMERIC(3,2) DEFAULT 0.00;
ALTER TABLE public.services ADD COLUMN IF NOT EXISTS search_vector tsvector GENERATED ALWAYS AS (setweight(to_tsvector('french', coalesce(title, '')), 'A') || setweight(to_tsvector('french', coalesce(description, '')), 'B')) STORED;
...
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS commission_amount NUMERIC(12,2) DEFAULT 0;
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS currency_code CHAR(3) DEFAULT 'XAF' REFERENCES public.currencies(code);
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS billing_address JSONB;

-- Upgrade ORDER_ITEMS table
ALTER TABLE public.order_items ADD COLUMN IF NOT EXISTS service_id UUID REFERENCES public.services(id);
...
-- Upgrade MESSAGES table (from 00003_api_integrations.sql if it exists)
-- Wait, let's just make sure all columns are there
DO $$ BEGIN
...
DO $$ BEGIN
    ALTER TABLE public.messages ADD COLUMN IF NOT EXISTS type message_type DEFAULT 'text';
EXCEPTION WHEN duplicate_object OR undefined_table THEN null; END $$;
...
DO $$ BEGIN CREATE POLICY "Public can view verified companies" ON public.companies FOR SELECT USING (is_verified = true); EXCEPTION WHEN duplicate_object THEN null; END $$;
DO $$ BEGIN CREATE POLICY "Owners can manage their companies" ON public.companies FOR ALL USING (auth.uid() = owner_id); EXCEPTION WHEN duplicate_object THEN null; END $$;

ALTER TABLE public.kyc_requests ENABLE ROW LEVEL SECURITY;
DO $$ BEGIN CREATE POLICY "Owners can view their KYC" ON public.kyc_requests FOR SELECT USING (EXISTS (SELECT 1 FROM public.companies WHERE companies.id = kyc_requests.company_id AND companies.owner_id = auth.uid())); EXCEPTION WHEN duplicate_object THEN null; END $$;
DO $$ BEGIN CREATE POLICY "Owners can create KYC" ON public.kyc_requests FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM public.companies WHERE companies.id = kyc_requests.company_id AND companies.owner_id = auth.uid())); EXCEPTION WHEN duplicate_object THEN null; END $$;

ALTER TABLE public.products ENABLE ROW LEVEL SECURITY;
DO $$ BEGIN CREATE POLICY "Public can view active products" ON public.products FOR SELECT USING (is_active = true); EXCEPTION WHEN duplicate_object THEN null; END $$;
DO $$ BEGIN CREATE POLICY "Sellers can manage their products" ON public.products FOR ALL USING (EXISTS (SELECT 1 FROM public.companies WHERE companies.id = products.company_id AND companies.owner_id = auth.uid())); EXCEPTION WHEN duplicate_object THEN null; END $$;

ALTER TABLE public.orders ENABLE ROW LEVEL SECURITY;
DO $$ BEGIN CREATE POLICY "Buyers can view their orders" ON public.orders FOR SELECT USING (auth.uid() = buyer_id); EXCEPTION WHEN duplicate_object THEN null; END $$;
DO $$ BEGIN CREATE POLICY "Sellers can view their orders" ON public.orders FOR SELECT USING (EXISTS (SELECT 1 FROM public.companies WHERE companies.id = orders.company_id AND companies.owner_id = auth.uid())); EXCEPTION WHEN duplicate_object THEN null; END $$;

ALTER TABLE public.wallets ENABLE ROW LEVEL SECURITY;
DO $$ BEGIN CREATE POLICY "Users can view their wallets" ON public.wallets FOR SELECT USING (auth.uid() = owner_id); EXCEPTION WHEN duplicate_object THEN null; END $$;

-- System Admin bypass
DO $$ BEGIN CREATE POLICY "Admins bypass RLS" ON public.companies FOR ALL USING (EXISTS (SELECT 1 FROM public.profiles WHERE profiles.id = auth.uid() AND profiles.role = 'admin')); EXCEPTION WHEN duplicate_object THEN null; END $$;
