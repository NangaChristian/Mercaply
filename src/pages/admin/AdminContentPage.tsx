...
export function AdminContentPage() {
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>, type: 'slide' | 'slide-cover' | 'banner') => {
    try {
      ...
      // using base64 for simplicity if supabase is not available
          ...
          {!isEditing ? (
                    ...
                    {slides.map((slide) => (
                      <tr key={slide.id}>
                        <td className="px-6 py-4">
                          <img src={slide.image_url} alt={slide.title} className="h-16 w-32 object-cover rounded" />
                        </td>
                        <td className="px-6 py-4">
            ...
            <div className="bg-white p-6 rounded-xl shadow max-w-2xl">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium mb-1">Image de fond (URL ou Upload)</label>
                  ...
                  {currentSlide.image_url && <img src={currentSlide.image_url} alt="Preview" className="mt-2 h-32 rounded object-cover" />}
                </div>

                <div>
                  <label className="block text-sm font-medium mb-1">Image de premier plan (Optionnelle)</label>
                  <div className="flex gap-2">
                    <input 
                      type="text" 
                  ...
                  {currentSlide.cover_image_url && <img src={currentSlide.cover_image_url} alt="Preview" className="mt-2 h-32 rounded object-contain bg-gray-100" />}
                </div>

                <div>
                  <label className="block text-sm font-medium mb-1">Surtitre (jaune)</label>
                  <input 
                    type="text" 
      ...
      {activeTab === 'banners' && (
        <div>
          {!isEditing ? (
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {banners.map((banner) => (
                      <tr key={banner.id}>
                        ...
                        <td className="px-6 py-4 flex gap-2">
                          <button onClick={() => { setCurrentBanner(banner); setIsEditing(true); }} className="text-blue-600 hover:text-blue-800">
                            <Edit2 size={18} />
                          </button>
            ...
            <div className="bg-white p-6 rounded-xl shadow max-w-2xl">
              <h2 className="text-xl font-bold mb-4">{currentBanner.id ? 'Modifier la Bannière' : 'Nouvelle Bannière'}</h2>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  {currentBanner.image_url && <img src={currentBanner.image_url} alt="Preview" className="mt-2 h-32 rounded object-cover" />}
                </div>
