...
const regex = /const handleConfirmOrder = async \(\) => \{[\s\S]+?\}\s*\} catch \(error\) \{[\s\S]+?finally \{\s*setIsProcessing\(false\);\s*\}\s*\};/;
...
content = content.replace(regex, confirmOrderReplacement);
